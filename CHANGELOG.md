...
|----------|-----|---------|
| **Pearson Math** | Replace `torch.std()` with safe `torch.sqrt(var + 1e-8)` | **Infinite Gradient Trap**: `torch.std(p)` evaluates to an infinite derivative around $0$. Even trailing bounds cannot save the backward pass from being polluted. Doing the math manually guarantees gradient bounds everywhere. |
| **Code Cleanup** | Remove duplicate `forward` in `ChannelAttention` | **Redundancy**: Fix duplicate methods caused by previously overlapping code replacements, keeping models slim and strict. |

---

## [2026-03-30 16:15] V8.0 Hotfix 2: AMP FP16 Explosion Guards
> **Core**: Fix NaNs generated by FP16 subnormal value flushing in Pearson loss and unbounded FiLM scaling.

### Fixes (V8.0 Patch 2)
| Category | Fix | The Why |
|----------|-----|---------|
| **FiLM Bounds** | `torch.tanh(gamma)` | **Prevent Affine Explosion**: Raw MLP output can be arbitrarily large. Bounding $\gamma$ to $[-1, 1]$ prevents feature amplification from overflowing the FP16 `MAX_VAL` limit. |
| **Pearson Guard** | `std < 1e-4` intercept | **FP16 Division By Zero**: In FP16, `1e-8` is mathematically zero. Flat patches resulted in division by exactly zero. Bumping the threshold to FP16's subnormal limit (`1e-4`) prevents NaN metric collapse. |
...
|-----------|--------|---------|
| **Temporal Context** | Single MODIS -> 3-sequence ($T_{-1}, T_0, T_{+1}$) | **Robustness**: 5x5 MODIS $T_0$ is prone to cloud noise. Sequence input + `TemporalContextAggregator` allow the model to learn growth trends and smooth out anomalies. |
| **FiLM Modulation** | Concatenated 2D channels -> 1D `meta_features` | **Feature Preservation**: DeltaT and DOY are global constants. Separating them into FiLM $(\gamma, \beta)$ prevents these "global knobs" from being lost in deep conv layers. |
| **Consistent Target** | $L_{cons}$ targets `modis_agg.detach()` | **Clean Supervision**: Instead of forcing the model to match potentially clouded $T_0$, we supervise with the temporal-smoothed "trend" generated by the aggregator. |
| **Input Shape** | `IN_CHANNELS`: 8 -> 5 | **Efficiency**: Moved 3 metadata channels out of the 2D image stack into the FiLM MLP branch. |
